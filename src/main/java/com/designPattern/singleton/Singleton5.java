...
/**
 * Singleton - 5. 单件注册表(registry of singleton)
...
//	private Singleton3(){}	//私有化构造方法， 这样别的类就没法调到。
	...
	static{
		try{
			InputStream is = Singleton5.class.getClassLoader().getResourceAsStream("resource/singletonConfig.properties");
	...
	protected static Singleton5 lookup(){
		return lookup(null);
	}
	protected static Singleton5 lookup(String keyName){
		Singleton5 singleton = null;
		try{
			if(keyName == null){
			...
			if(className != null){
				...
				singleton = (Singleton5) c.newInstance();
			}else{
	...
	public static void registerSingleton(String keyName, Singleton5 singleton){
